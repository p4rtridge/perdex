...
use crate::cavage::header::{SignatureHeader, lexer};
...
/// Deserialize a signature header string into a `SignatureHeader` struct
pub fn derserialize(
    input: &str,
) -> Result<SignatureHeader<'_, impl Iterator<Item = &'_ str> + Clone>, Report<SerdeError>> {
    ...
    while let Some((key, value)) = tokenizer
        .next()
        .transpose()
        .change_context(SerdeError::InvalidHeaderFormat)?
    {
        match key {
            ...
            "created" => builder.created(
                value
                    .parse::<u64>()
                    .change_context(SerdeError::Radix10Parse)?,
            ),
    ...
    builder.build().change_context(SerdeError::BuildFailed)
}
