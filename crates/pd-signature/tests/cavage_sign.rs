...
pub fn get_request() -> Request<()> {
    Request::builder()
        .method(Method::POST)
        .uri(Uri::from_static("/partridge?param=value&is-it-cool=true"))
        .header("Host", "example.org")
        .header("Date", "Sun, 21 Apr 2026 11:31:40 GMT")
...
fn basic() {
    ...
    let signed_request = cavage::sig::sign(req, "Test", &get_pkcs8_private_key()).unwrap();
