...
fn get_rsa_public_key() -> Vec<u8> {
    let pem = include_str!("../../../test-fixtures/keys/public_rsa.pem");
...
fn get_rsa_private_key() -> Vec<u8> {
    let pem = include_str!("../../../test-fixtures/keys/private_rsa.pem");
...
fn get_ed25519_public_key() -> Vec<u8> {
    let pem = include_str!("../../../test-fixtures/keys/public_ed25519.pem");
    let (_tag, document) = Document::from_pem(pem).unwrap();
    document.as_bytes().to_vec()
}
...
fn get_ed25519_private_key() -> Vec<u8> {
    let pem = include_str!("../../../test-fixtures/keys/private_ed25519.pem");
    let (_tag, document) = SecretDocument::from_pem(pem).unwrap();
    document.as_bytes().to_vec()
}
